...
def _setoptions(options):
    ...
    # int options
    inttype = ['gpu', 'kernel_size', 'max_order', 'xn', 'yn', 'interp_degree', 'interp_mesh_size', 
            'initfreq', 'batch_size', 'teststepnum', 'maxiter', 'recordcycle', 'savecycle', 'repeatnum']
    options = _options_cast(options, inttype, int)
    # float options
    floattype = ['dt', 'variant_coe_magnitude', 'start_noise_level', 'end_noise_level']
...
class callbackgen(object):
    ...
    def load(self, l):
        ...
        size = self.module.coe0.inputs_size
        xy = self.module.xy.clone()
        if 'coe0._inputs' in params:
            self.module.xy = Variable(params['coe0._inputs'])
...
def setenv(options):
    ...
    linpdelearner = pdelearner.VariantCoeLinear2d(kernel_size=namestobeupdate['kernel_size'],max_order=namestobeupdate['max_order'],dx=2*np.pi/namestobeupdate['mesh_size'],constraint=namestobeupdate['constraint'],xy=xy,interp_degree=namestobeupdate['interp_degree'],interp_mesh_size=namestobeupdate['interp_mesh_size'],dt=namestobeupdate['dt']) # build pde-net for variant coefficient pde
